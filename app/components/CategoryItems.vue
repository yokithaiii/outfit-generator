                    ...
                    <div class="l-items l-max-4 gap-4 mt-4">
                        <div class="h-full flex flex-col justify-center items-center">
                            <UModal v-model:open="modalsOpen" title="Загрузить изображение">
                                <div class="l-card h-full w-full flex justify-center items-center cursor-pointer min-h-[200px]"
                                    @click="openModal">
                                    <UIcon name="ant-design:cloud-upload-outlined" size="large" class="w-[40px] h-[40px]">
                                    </UIcon>
                                </div>
                                <template #body>
                                    <UFileUpload label="Drop your image here" class="w-full h-[400px] cursor-pointer"
                                        v-model="image" />
                        ...
                        <template v-if="props.loading">
                            <div class="grid grid-cols-4 gap-4 w-full">
                                <div class="l-card h-full w-full" v-for="n in 4" :key="n">
                                    <USkeleton class="h-[200px] w-full" style="border-radius: 0;" />
                                </div>
                            </div>
                        </template>
                        <UCarousel v-else v-slot="{ item }" :items="localItems" :ui="{ item: 'basis-1/4' }"
                            :loading="loading" class="w-full mx-auto">
                            <div class="l-card" @click="openImgModal(item)">
                                <img :src="item.image_url" alt="" class="h-[200px] w-full object-cover" />
                            </div>
                        </UCarousel>
                    </div>
